...
void file_thread_function (string filename, BoundedBuffer* bufreq, int m, TCPRequestChannel* chan) {
...
void worker_thread_function (BoundedBuffer* bufreq, BoundedBuffer* bufresp, TCPRequestChannel* chan, int m) {
...
int main (int argc, char* argv[]) {
	...
	TCPRequestChannel* chan = new TCPRequestChannel(x,y);
    ...
    vector<TCPRequestChannel*> fifo(w);
    ...
    if (f !=""){
       // FIFORequestChannel* newchan = new FIFORequestChannel("control", FIFORequestChannel::CLIENT_SIDE);
        ...
        newchan = new TCPRequestChannel(x,y);
    ...
    for (int i=0; i < w ; ++i ){
            ...
            // char nextbuf[m];
            // chan->cread(&nextbuf, m);
            fifo.at(i) = new TCPRequestChannel(x, y);
